...
def create_sessions() -> requests.Session:
    ...
    session.mount("htpps://", adapter)

    return session

def fetch_page(url: str, session: requests.sessions) -> Optional[BeautifulSoup]:
    ...
    try:
        ...
        for idx, block in enumerate(question_blocks, 1):
            try:
                ...
                answer_div = block.find('div', class_='bix_answer_explian')
                answer = "Not available"
                explanation = ""

                if answer_div:
                    answer_span = answer_div.find('span', class_='jd-hdnakqb')
                    if answer_span:
                ...
                logger.error(f"Error extracting questions {idx}: {str(e)}")
                continue
...
def scrape_date_wise(date_obj, session: requests.Session) -> List[Dict]:
    ...
    date_url = f"{CURRENT_AFFAIRS_URL}{date_obj.year}/{date_obj.strftime('%B').lower()}/{date_obj.day}/"
    ...
    for question in questions:
        question['data'] = date_obj.strftime('%Y-%m-%d')
